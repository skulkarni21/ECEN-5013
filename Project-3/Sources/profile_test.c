 ...
 *
 *      Author: Sarang Kulkarni, Yu-Chih Cho
 *
 ****************************************************************************/
...
void DMA_profile_test(void){
	 uint8_t *ucStrAddr = (uint8_t *)0x20000000;
	 uint8_t *ucDstAddr = (uint8_t *)0x20001000;
	 uint32_t i         = 0;
	 uint8_t  ucTemp    = 0;
	 ...
	 *ucStrAddr = 0 ; //need to declare if use memzero
/*-----------------------------------For 10 bytes-------------------------------------------*/
	 start_timer();
	 dmaInitMemToMem(MKL_DMA0, ucStrAddr, ucDstAddr, 10, 0);           // 0 tunnel, no interrupt
	 dmaStartTran(MKL_DMA0);                                             // DMA start to transfer
	 while (dmaStatus(MKL_DMA0) != DMA_DONE);                            // wait until DMA transfer is done
	 DMA_stop_time = stop_timer();
	 //log3("\nTime for DMA_memmove(10 bytes) = ",DMA_stop_time);
	 log3("\nTime for DMA_memzero(10 bytes) = ",DMA_stop_time);
	 write_string("uSec");

/*-----------------------------------For 100 bytes-------------------------------------------*/
	 start_timer();
	 dmaInitMemToMem(MKL_DMA0, ucStrAddr, ucDstAddr, 100, 0);           // 0 tunnel, no interrupt
	 dmaStartTran(MKL_DMA0);                                             // DMA start to transfer
	 while (dmaStatus(MKL_DMA0) != DMA_DONE);                            // wait until DMA transfer is done
	 DMA_stop_time = stop_timer();
	 //log3("\nTime for DMA_memmove(100 bytes) = ",DMA_stop_time);
	 log3("\nTime for DMA_memzero(100 bytes) = ",DMA_stop_time);
	 write_string("uSec");

/*-----------------------------------For 1000 bytes-------------------------------------------*/
	 start_timer();
	 dmaInitMemToMem(MKL_DMA0, ucStrAddr, ucDstAddr, 1000, 0);           // 0 tunnel, no interrupt
	 dmaStartTran(MKL_DMA0);                                             // DMA start to transfer
	 while (dmaStatus(MKL_DMA0) != DMA_DONE);                            // wait until DMA transfer is done
	 DMA_stop_time = stop_timer();
	 //log3("\nTime for DMA_memmove(1000 bytes) = ",DMA_stop_time);
	 log3("\nTime for DMA_memzero(1000 bytes) = ",DMA_stop_time);
	 write_string("uSec");

/*-----------------------------------For 5000 bytes-------------------------------------------*/
	 start_timer();
	 dmaInitMemToMem(MKL_DMA0, ucStrAddr, ucDstAddr, 5000, 0);           // 0 tunnel, no interrupt
 	 dmaStartTran(MKL_DMA0);                                             // DMA start to transfer
 	 while (dmaStatus(MKL_DMA0) != DMA_DONE);                            // wait until DMA transfer is done
	 DMA_stop_time = stop_timer();
	 //log3("\nTime for DMA_memmove(5000 bytes) = ",DMA_stop_time);
	 log3("\nTime for DMA_memzero(5000 bytes) = " ,DMA_stop_time);
	 write_string("uSec");

	 while (1);                                                          // wait until DMA transfer is done
...
/*-----------------------------------For 10 bytes-------------------------------------------*/
	...
	log3("\nTime for my_memmove(10 bytes) = ",stop_time);
	write_string("uSec");
...
/*----------------------------------For 100 bytes ------------------------------------------*/
	...
	stop_time =stop_timer();
	log3("\n\nTime for my_memmove(100 bytes) = ",stop_time);
	write_string("uSec");
...
/*----------------------------------For 1000 bytes ------------------------------------------*/
	...
	stop_time =stop_timer();
	log3("\nTime for memmove(1000 bytes) = ",stop_time);
	write_string("uSec");
...
/*----------------------------------For 5000 bytes ------------------------------------------*/
	...
	stop_time =stop_timer();
	log3("\n\nTime for my_memmove(5000 bytes) = ",stop_time);
	write_string("uSec");
	...
	stop_time =stop_timer();
	log3("\n\nTime for my_memzero(5000 chars) = ",stop_time);
	write_string("uSec");
